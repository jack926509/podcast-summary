...
export default function SubscriptionsPage() {
  ...
  return (
    <>
      ...
      {/* ── Body ──────────────────────────────────────────────────── */}
      <div className="mx-auto max-w-6xl space-y-6 px-5 py-4 md:px-10 md:py-6 md:space-y-8">
        {/* Empty state */}
        {podcasts && subscribed.length === 0 && (
          <div className="rounded-xl border border-dashed border-border bg-card px-6 py-12 text-center">
            ...
            <p className="mt-3 text-[13px] text-muted-foreground">尚未訂閱任何 Podcast。</p>
        ...
        {unsubscribed.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-border pb-2">
              <h2 className="font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">
                已解析（未訂閱）
              </h2>
            ...
            {unsubscribed.map((podcast) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13px] font-medium">{podcast.title}</p>
                  <p className="text-[11.5px] text-muted-foreground">
                    {podcast.author ?? ''}
                    {podcast.author && ' · '}
                    {podcast._count.episodes} 集已記錄
                  </p>
                </div>
                <Button
                  variant="outline"
...
function MobilePodcastTile({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      ...
      {expanded && (
        <div className="space-y-2 border-t border-border bg-muted/30 px-2.5 pb-3 pt-2">
          {/* Action buttons */}
...
function PodcastCard({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card transition hover:border-foreground/20">
      {/* Header */}
